    ...
    try {
      ...
      if (changed.length > 0) {
        ...
        notificationController.show({
          ...
          message: `可见性变为 ${count} ${count <= 1 ? '人' : '人'}`,
        });
      }
    } catch (error) {
      handleError(
        error,
        `无法更改可见性 ${changed.length} ${changed.length <= 1 ? '人' : '人'}`,
      );
    }
    // Reset variables used on the "Show & hide people" modal
    showLoadingSpinner = false;
  ...
  {:else}
    <div class="flex min-h-[calc(66vh_-_11rem)] w-full place-content-center items-center dark:text-white">
      <div class="flex flex-col content-center items-center text-center">
        ...
        <p class="mt-5 text-3xl font-medium">无人物</p>
      </div>
    </div>
  {/if}
  ...
  {#if showChangeNameModal}
    <FullScreenModal on:clickOutside={() => (showChangeNameModal = false)}>
      ...
      >
        ...
        >
          <h1 class="text-2xl font-medium text-immich-primary dark:text-immich-dark-primary">修改名称</h1>
        </div>
