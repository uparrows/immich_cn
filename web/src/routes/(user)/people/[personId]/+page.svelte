<script lang="ts">
  ...
  const handleSelectFeaturePhoto = async (asset: AssetResponseDto) => {
    ...
    notificationController.show({ message: '专题照片已更新', type: NotificationType.Info });
    assetInteractionStore.clearMultiselect();
  ...
  const handleSetBirthDate = async (birthDate: string) => {
    try {
      ...
      notificationController.show({ message: '出生日期保存成功', type: NotificationType.Info });
    } catch (error) {
      handleError(error, '无法保存出生日期');
    }
  };
  ...
  {#if $isMultiSelectState}
    <AssetSelectControlBar assets={$selectedAssets} clearSelect={() => assetInteractionStore.clearMultiselect()}>
      ...
      <AssetSelectContextMenu icon={mdiDotsVertical} title="添加">
        ...
        <MenuOption text="修复错误匹配" on:click={handleReassignAssets} />
        <ChangeDate menuItem />
      ...
      <ControlAppBar showBackButton backIcon={mdiArrowLeft} on:close={() => goto(previousRoute)}>
        <svelte:fragment slot="trailing">
          <AssetSelectContextMenu icon={mdiDotsVertical} title="菜单">
            <MenuOption text="更改专题照片" on:click={() => (viewMode = ViewMode.SELECT_PERSON)} />
            <MenuOption text="设定出生日期" on:click={() => (viewMode = ViewMode.BIRTH_DATE)} />
              ...
              <div class="relative">
                ...
                >
                  ...
                  >
                    ...
                    {:else}
                      <p class="font-medium">添加名称</p>
                      <p class="text-sm text-gray-500 dark:text-immich-gray">通过搜索名称快速找到它们</p>
                    {/if}
                  </div>
                </button>
              </div>
