<script lang="ts">
  ...
  const handleUpdateThumbnail = async (assetId: string) => {
    ...
    try {
      ...
      notificationController.show({ type: NotificationType.Info, message: '更新相册封面' });
    } catch (error) {
